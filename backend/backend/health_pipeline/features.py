...
def extract(
    ...
    """Top-level entry point. Caller must supply `df` (the wearable dataframe).
    `start` / `end` default to the dataframe's full index."""
    if df is None:
        raise ValueError("extract() requires a `df` argument")
    if start is None:
        start = df.index[0]
    if end is None:
        end = df.index[-1]
