...
from health_pipeline.config import DEEPSEEK_API_KEY, DEEPSEEK_BASE_URL, DEEPSEEK_MODEL
...
class DeepSeekBackend:
    ...
    def synthesize(
        ...
        sys_p = system_prompt(domain, tone)
        context = render_chunks(chunks)
        data = render_data_block(features, narration)
        ...
        parsed, err = _validate(domain, raw)
        ...
        parsed2, err2 = _validate(domain, raw2)
...
def _validate(domain: "Domain", raw: str) -> tuple[dict | None, str]:
    try:
        data = json.loads(raw)
    except json.JSONDecodeError as e:
        return None, f"JSONDecodeError: {e}"
    try:
        validated = domain.response_model.model_validate(data)
